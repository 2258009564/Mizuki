...
updated: 2026-03-12T16:54:24.123Z # 最后更新时间 (格式: YYYY-MM-DDTHH:mm:ss.sssZ)
--- -->

# 写在之前
在此之前，我尝试在 `GitHub pages` 上部署过一个基于 `Quartz` 的 [静态知识库](https://2258009564.github.io/)，但由于 `Quartz` 的功能过于简单，无法满足我对博客的需求（臆想）。

再后来，我也尝试过使用 `Hexo` 和 `Hugo` 等静态博客生成器，但由于它们的配置较为复杂，且需要一定的前端知识，加上我的懒癌，导致最后没有好好操作下去。
...
完成了`GitHub` 和 `Cloudflare` 的部署后，所有需要动脑的工作告一段落，接下来我们就可以专注于博客的内容和配置了。
