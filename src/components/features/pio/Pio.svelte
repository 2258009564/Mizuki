<script>
	...
	const configuredModels =
		Array.isArray(pioConfig.models) && pioConfig.models.length > 0
			? pioConfig.models
			: ["/pio/UG/ugofficial.model3.json"];

	// 灏嗛厤缃浆鎹负 Pio 鎻掍欢闇€瑕佺殑鏍煎紡
	const pioOptions = {
		mode: pioConfig.mode,
	...
	function pickTouchDialogText() {
		...
		if (!Array.isArray(source) || source.length <= 1) {
			const text = pickDialogText(source, "~");
	...
	async function restoreCubismModel(event) {
		...
		showCubismDialog(
			pickDialogText(pioConfig.dialog?.welcome, "Welcome back~"),
			2500,
		);
		keepPioInsideViewport();
	}
	...
	async function loadCubismModel(modelPath) {
		...
		try {
			...
			showCubismDialog("Model load failed", 4000);
			console.error("Cubism4 failed, no legacy fallback");
			return false;
		}
	}
	...
	function setupCubismActions() {
		...
		homeButton.onmouseover = () =>
			showCubismDialog(
				pickDialogText(pioConfig.dialog?.home, "Click to go home~"),
			);
		...
		infoButton.onmouseover = () =>
			showCubismDialog("Want to know more?");
		...
		closeButton.onmouseover = () =>
			showCubismDialog(
				pickDialogText(pioConfig.dialog?.close, "~"),
			);
	...
	async function initCubism4Pio() {
		...
		if (!cubismPIXI || !cubismLive2DModel) {
			...
			if (!ready) {
				showCubismDialog("Cubism4 core failed", 4200);
				console.error("Cubism4 failed, no legacy fallback");
				return;
			}
		...
		try {
			...
			if (loaded) {
				...
				keepPioInsideViewport();
				showCubismDialog(
					pickDialogText(pioConfig.dialog?.welcome, "~"),
					2600,
				);
				console.log(
			...
			showCubismDialog("Cubism4 init failed", 3800);
			console.error("Cubism4 failed, no legacy fallback");
		}
	}
	function loadPioAssets() {
		...
		ensureCubism4Runtime()
			...
			.catch((error) => {
				...
				showCubismDialog("Cubism4 runtime error", 3800);
			});
	}

	// 鏍峰紡宸查€氳繃 Layout.astro 闈欐€佸紩鍏ワ紝鏃犻渶椤甸潰鍒囨崲鐩戝惉
...
{#if pioConfig.enable}
	<div
		class={`pio-container ${pioConfig.position || "right"} cubism4-mode ${showAnchorX === "right" ? "show-right" : ""} ${showAnchorY === "top" ? "show-top" : ""}`}
